pub fn kangarootwelve(input: &[u8], customization: &[u8], output: &mut [u8]) {
    let ret = unsafe {
        KangarooTwelve(
            input.as_ptr(),
            input.len(),
...
mod test {
    ...
    fn test_vector_03() {
        ...
        let mut out = [0; 10032];
        kangarootwelve(&[], &[], &mut out);
        assert_eq!(expected, hex::encode(&out[out.len() - 32..]));
    }
    ...
    fn test_vector_11() {
        ...
        let mut out = [0; 32];
        kangarootwelve(&[], &customization, &mut out);
        assert_eq!(expected, hex::encode(&out));
    }
    ...
    fn test_vector_12() {
        ...
        fill_pattern(&mut customization);
        let mut out = [0; 32];
        kangarootwelve(&input, &customization, &mut out);
        assert_eq!(expected, hex::encode(&out));
    }
