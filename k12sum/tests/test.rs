...
fn test_null_device_on_windows() {
    let expected = hash(b"").to_hex();
    let output = cmd!(k12sum_exe(), "--no-names", "NUL").read().unwrap();
    assert_eq!(expected.as_str(), output);
    let output = cmd!(k12sum_exe(), "--no-names", "--no-mmap", "NUL")
        .read()
        .unwrap();
    assert_eq!(expected.as_str(), output);
}
...
fn test_missing_files() {
    ...
    let output = cmd!(k12sum_exe(), "file1", "missing_file", "file2")
        .dir(dir.path())
        .stdout_capture()
        .stderr_capture()
        .unchecked()
    ...
    let foo_hash = kangarootwelve_xkcp::hash(b"foo");
    let bar_hash = kangarootwelve_xkcp::hash(b"bar");
    ...
    let expected_stderr = format!("k12sum: missing_file: {}\n", bing_error.to_string());
